...
parser.add_argument('--m_in', type=int, default=-15)
parser.add_argument('--m_out', type=int, default=-5)
...
oe_loader = DataLoader(
    oe_set, batch_size=2*args.batch_size, shuffle=True, 
    num_workers=args.prefetch, pin_memory=False
)

# Set up checkpoint directory and tensorboard writer
if args.save_dir is None:
    oe_related = f'oe_{args.oe_set}'
    if args.energy:
        oe_related += f'_energy_min={args.m_in}_mout={args.m_out}'
...
def train():
    ...
    for x, y in batch_iter:
        ...
        if args.energy:
            Ec_out = -torch.logsumexp(logits[batch_size:], dim=1)
            Ec_in = -torch.logsumexp(logits[:batch_size], dim=1)
            loss += args.beta * (torch.pow(F.relu(Ec_in-args.m_in), 2).mean() + torch.pow(F.relu(args.m_out-Ec_out), 2).mean())
        else:
